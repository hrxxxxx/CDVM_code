...
class BlockWiseMaskGenerator:
    def __init__(
            ...
            min_aspect=0.3, max_aspect=None, patch_size=16):
    ...
    def __call__(self, target=-1, foreground_mask=None, image=None, sampling_weights=None):
        ...
        if foreground_mask is not None:  # masking at foreground
            fg_mask = patchify(foreground_mask, patch_size=self.patch_size, num_chans=1)
    ...
    def _masking(self, sampling_weights=None, image=None, fg_mask=None):
        ...
        if image is not None:
            ...
            freq = torch.view_as_real(torch.fft.fft(image, dim=2, norm='ortho'))
            freq = torch.sqrt(freq[..., 0]**2 + freq[..., 1]**2)
            freq = freq / freq.max()
            freq[torch.isnan(freq)] = 0.0
            freq = torch.clamp(freq, min=0.0, max=1.0)
            freq = patchify(freq, patch_size=self.patch_size)
            freq = freq.sum(-1)
        else:
            freq = None
    ...
    def _mask(self, mask, max_mask_patches, sampling_weights=None, freq=None, fg_mask=None):
        ...
        """
        delta = 0
        idxes = np.arange(self.num_patches).reshape(self.width, self.height)
        if fg_mask is not None:
            fg_mask = fg_mask.reshape(self.width, self.height).numpy()
            # print(np.sum(fg_mask))
        if sampling_weights is not None:
            weights = sampling_weights.reshape(self.width, self.height)
            weights = weights.numpy()
        if freq is not None:
            weights = freq.reshape(self.width, self.height)
            weights = weights.numpy()
...
class RandomMaskGenerator:
    def __init__(
        self, input_size, num_masking_patches, patch_size=16):
        if not isinstance(input_size, tuple):
            input_size = (input_size, ) * 2
        self.height, self.width = input_size
        self.patch_size = patch_size
    ...
    def _masking_by_frequency(self, image, num_patches, fg_mask=None):
        # freq = torch.fft.rfft(image, 2, onesided=False, normalized=True)
        freq = torch.view_as_real(torch.fft.fft(image, dim=2, norm='ortho'))
        freq = torch.sqrt(freq[..., 0]**2 + freq[..., 1]**2)
        freq = freq / freq.max()
        freq[torch.isnan(freq)] = 0.0
