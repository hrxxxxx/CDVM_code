...
class VISA(Dataset):
    ...
    def __init__(
        ...
        if not multi_road:
            ...
            if kwargs.get('tokenizer_model') == 'dall-e':
                self.token_transform16 = T.Compose([
                    T.Resize(112, Image.ANTIALIAS),
                    T.CenterCrop(112),
                    T.ToTensor(),
                    map_pixels])

                self.token_transform32 = T.Compose([
                    T.Resize(56, Image.ANTIALIAS),
                    T.CenterCrop(56),
                    T.ToTensor(),
                    map_pixels])
            else:
    ...
    def __getitem__(self, idx: int) -> Tuple[Any, Any, Any]:
        ...
        if not self.multi_road:
            token_image = self.token_transform(image)
        else:
            token_image16, token_image32 = self.token_transform16(image), self.token_transform32(image)
        ...
        if not self.multi_road:
            ...
            return image, token_image16, token_image32, label, mask, os.path.basename(image_path[:-4]), img_type
    ...
    def download(self) -> None:
        ...
        if not os.path.exists(tar_file_path):
            assert False, "should download visa data to local"
