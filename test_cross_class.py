import warnings
from train_cross_class import get_size_info, get_tokenizer
...
def get_amap(init_size, mask, uncertainty, window_size,  device):
    a_map = torch.zeros(1, init_size).to(device)
    a_map[mask == 1] = uncertainty
    a_map[mask == 0] = 0  # unmask patch, no anomaly score
    a_map = a_map.reshape(1, window_size, window_size)
    a_map = F.interpolate(a_map.unsqueeze(1), size=(224, 224), mode='bilinear', align_corners=True).squeeze()
    a_map = a_map.cpu()
    return a_map


def get_mask(mask, model, dtype):
    mask = mask.detach()
    mask = mask.unsqueeze(-1).repeat(1, 1, model.patch_embed.patch_size[0] ** 2 * 3)  # (N, H*W, p*p*3)
    mask = model.unpatchify(mask)  # 1 is removing, 0 is keeping
    mask = torch.einsum('nchw->nhwc', mask).detach().cpu()
...
def run_one_image_mr(img, mask16, mask32,  model, labels16, labels32):
    # run patch-level reconstruction model
    out16, out32, _, _ = model(img, img, mask16, mask32, return_all_tokens=False)

    uncertainty16 = xent_loss(input=out16, target=labels16)
    uncertainty32 = xent_loss(input=out32, target=labels32)

    # return a_map
    a_map16 = get_amap(14 * 14, mask16, uncertainty16, 14,  img.device)
    a_map32 = get_amap(7 * 7, mask32, uncertainty32, 7, img.device)
    a_map = (a_map16 + a_map32) /2

    # visualize the mask
    mask16 = get_mask(mask16, model.model16, img.dtype)
    mask32 = get_mask(mask32, model.model32, img.dtype)

    x = torch.einsum('nchw->nhwc', img).detach().cpu()

    # masked image
    im_masked16 = x * (1 - mask16)
    im_masked32 = x * (1 - mask32)
    # sum mask img
    im_masked = (im_masked16 + im_masked32) / 2
...
def evaluation_mr(args16, args32, model, tokenizer16, tokenizer32, protoflow):
    device = torch.device(args16.device)

    dataset = MVTEC(args16.data_path, class_name=args16.class_name, train=False,
                    norm_mean=[0.485, 0.456, 0.406], norm_std=[0.229, 0.224, 0.225], img_size=224, crp_size=224,
                    tokenizer_model=args16.tokenizer_model, multi_road=True)
    data_loader = DataLoader(dataset, batch_size=1, shuffle=False, drop_last=False)

    image_aucs, pixel_aucs = [], []
    pre_image_aucs, pre_pixel_aucs = [], []
    for mask_ratio in np.arange(0.02, 0.5, 0.02).tolist():
        mask_generator16 = RandomMaskGenerator(
            args16.window_size, num_masking_patches=int(args16.patch_size[0] * args16.patch_size[1] * mask_ratio))
        mask_generator32 = RandomMaskGenerator(
            args32.window_size, num_masking_patches=int(args32.patch_size[0] * args32.patch_size[1] * mask_ratio))

        pre_masks16, pre_masks32, img_labels, abnomalities = protoflow.predict(data_loader, args16.class_name, mask_ratio, multi_road=True)
        ...
        img_types = []
        ori_img_list, msk_img_list = [], []
        for i, (img, token_img16, token_img32, label, mask, _, img_type) in enumerate(
                tqdm(data_loader, 'Evaluation Dataset->{}.{}'.format('MVTEC', args16.class_name))):
            gt_label_list.append(label.cpu().numpy())
            gt_mask_list.append(mask.squeeze().cpu().numpy())
            img_types.append(img_type)

            img = img.to(device, non_blocking=True)
            token_img16 = token_img16.to(device, non_blocking=True)
            token_img32 = token_img32.to(device, non_blocking=True)
            with torch.no_grad():
                mask16, mask32 = pre_masks16[i], pre_masks32[i]
                mask16 = torch.from_numpy(mask16).to(device).to(torch.long)
                mask32 = torch.from_numpy(mask32).to(device).to(torch.long)
                mask16, mask32 = mask16.unsqueeze(0), mask32.unsqueeze(0)
                img_label = img_labels[i]
                mask16 = generate_mask(mask16, mask_generator16, label=img_label)
                mask32 = generate_mask(mask32, mask_generator32, label=img_label)

                input_ids16 = tokenizer16.get_codebook_indices(token_img16).flatten(1)
                input_ids32 = tokenizer32.get_codebook_indices(token_img32).flatten(1)
                mask16 = mask16.reshape(1, 196).to(torch.bool)
                mask32 = mask32.reshape(1, 49).to(torch.bool)

                labels16 = input_ids16[mask16]
                labels32 = input_ids32[mask32]

                ori_img, msk_img, score = run_one_image_mr(img, mask16, mask32,  model, labels16, labels32)
                score = score.numpy()
                score = score * abnomalities[i]
                ori_img_list.append(ori_img)
                msk_img_list.append(msk_img)
        ...
        scores = np.stack(score_list, axis=0)
        labels = np.asarray(gt_label_list)
        masks = np.asarray(gt_mask_list, dtype=np.bool_)
        img_scores = np.max(scores.reshape(scores.shape[0], -1), axis=-1)
        image_auc, pixel_auc, _ = measure_rocaucs(labels, masks, img_scores, scores)
        print(f'Category {args16.class_name}: Image-AUC: {image_auc}, Pixel AUC: {pixel_auc}, Mask Ratio: {mask_ratio}')
        image_aucs.append(image_auc)
        pixel_aucs.append(pixel_auc)
...
def parse_args():
    ...
    parser.add_argument('--model_path', type=str,
        default='output_dir/cross_class/vit_base_16_lr5e-5_freq50_MVTEC_to_texture_to_obj_v2/checkpoint-499.pth', help='checkpoint path of model')
...
def main():
    ...
    # Evaluate on UNSEEN_TEXTURES
    args.model_path = 'output_dir/cross_class/vit_base_16_lr5e-5_freq50_MVTECobj_to_texture_v2/checkpoint-299.pth'
    ...
    protoflow = ProtoFlow('weights/protoflow/weights_texture_to_obj.pt')
    

    all_image_aucs_texture, all_pixel_aucs_texture, mask_ratios_texture = [], [], []
    for class_name in UNSEEN_TEXTURES:
        args16.class_name = class_name

        image_auc, pixel_auc, mask_ratio = evaluation_mr(args16, args32, model, tokenizer16, tokenizer32, protoflow)
        print('Category: {}'.format(class_name))
        print("Max Image-AUC: {}".format(image_auc))
        print("Max Pixel-AUC: {}".format(pixel_auc))
        all_image_aucs_texture.append(image_auc)
        all_pixel_aucs_texture.append(pixel_auc)
        mask_ratios_texture.append(mask_ratio)

    for i, class_name in enumerate(UNSEEN_TEXTURES):
        print(f'{class_name}: Image-AUC: {all_image_aucs_texture[i]}, Pixel-AUC: {all_pixel_aucs_texture[i]}, Mask-Ratio: {mask_ratios_texture[i]}')
    print('Mean Image-AUC (Textures): {}'.format(np.mean(all_image_aucs_texture)))
    print('Mean Pixel-AUC (Textures): {}'.format(np.mean(all_pixel_aucs_texture)))
    
    # Evaluate on UNSEEN_OBJECTS
    args16, args32 = copy.deepcopy(args), copy.deepcopy(args)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    args.model_path = 'output_dir/cross_class/vit_base_16_lr5e-5_freq50_MVTEC_to_texture_to_obj_v2/checkpoint-499.pth'
    args.device = device
    ...
    protoflow = ProtoFlow('weights/protoflow/weights_obj_to_texture.pt')


    all_image_aucs_object, all_pixel_aucs_object, mask_ratios_object = [], [], []
    for class_name in UNSEEN_OBJECTS:
        ...
        print("Max Image-AUC: {}".format(image_auc))
        print("Max Pixel-AUC: {}".format(pixel_auc))
        all_image_aucs_object.append(image_auc)
        all_pixel_aucs_object.append(pixel_auc)
        mask_ratios_object.append(mask_ratio)

    for i, class_name in enumerate(UNSEEN_OBJECTS):
        print(f'{class_name}: Image-AUC: {all_image_aucs_object[i]}, Pixel-AUC: {all_pixel_aucs_object[i]}, Mask-Ratio: {mask_ratios_object[i]}')
    print('Mean Image-AUC (Objects): {}'.format(np.mean(all_image_aucs_object)))
    print('Mean Pixel-AUC (Objects): {}'.format(np.mean(all_pixel_aucs_object)))
