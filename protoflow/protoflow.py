...
class ProtoFlow(object):
    def __init__(self, path, device='cuda'):
        ...
        encoder = timm.create_model('tf_efficientnet_b6', features_only=True,
                out_indices=(2, 3), pretrained=True, pretrained_cfg_overlay={'file': '/root/autodl-tmp/PMAD/protoflow/efficientnet-b6/tf_efficientnet_b6_aa-80ba17e4.pth'})
    ...
    def predict(self, data_loader, class_name, mask_ratio=0.3, multi_road=False):
        ...
        e_logp_list = []
        with torch.no_grad():
            for (image, _, _, label, mask, _, _) in data_loader:
        ...
        if not multi_road:
            pre_mask, img_labels, abnomality = get_outputs(prob, gt_label_list, rank, mask_ratio)
            return pre_mask, img_labels, abnomality
        else:
            pre_mask16, pre_mask12, img_labels, abnomality = get_outputs_mr(prob, gt_label_list, rank, mask_ratio)
            return pre_mask16, pre_mask12, img_labels, abnomality


def get_outputs(prob, gt_label_list, rank, mask_ratio):
...
def get_outputs_mr(prob, gt_label_list, rank, mask_ratio):
    rank16 = F.interpolate(rank.unsqueeze(1), size=14, mode='bilinear', align_corners=True).squeeze()
    rank32 = F.interpolate(rank.unsqueeze(1), size=7, mode='bilinear', align_corners=True).squeeze()
    prob = F.interpolate(prob.unsqueeze(1), size=224, mode='bilinear', align_corners=True).squeeze().cpu().numpy()
    abnomality = prob.max() - prob
    ...
    rank16 = rank16.reshape(-1, 196)
    rank32 = rank32.reshape(-1, 49)
    thres16 = torch.topk(rank16, max(int(196 * mask_ratio), 1), dim=-1)[0][:, -1]
    thres32 = torch.topk(rank32, max(int(49 * mask_ratio), 1), dim=-1)[0][:, -1]
    thres16, thres32 = thres16.unsqueeze(-1), thres32.unsqueeze(-1)
    thres16, thres32 = thres16.expand([thres16.shape[0], rank16.shape[-1]]), thres32.expand([thres32.shape[0], rank32.shape[-1]])
    pre_mask16 = rank16 >= thres16
    pre_mask32 = rank32 >= thres32

    pre_mask16, pre_mask32 = pre_mask16.cpu().numpy(), pre_mask32.cpu().numpy()

    return pre_mask16, pre_mask32, img_labels, abnomality
